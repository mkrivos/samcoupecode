...
/// \date 5. 6. 2026
/// \brief CLI + round-trip self-test for the IMPLODE LZ77 codec.
///
...
{
    auto packed = implode::compress(in);
    auto back = implode::decompress(packed);
    bool ok = (back == in);
...
{
    int fails = 0;
    std::printf("IMPLODE LZ77 codec self-test (round-trip):\n");
    fails += roundTrip("empty", {});
    fails += roundTrip("single byte", {0x42});
    fails += roundTrip("marker 0x03 byte", {0x03});
    fails += roundTrip("marker run", {0x03, 0x03, 0x03, 0x03, 0x03});
    {
        std::vector<std::uint8_t> v(8000, 0xAA);            // long run -> matches
        fails += roundTrip("all 0xAA 8k", v);
    }
    {
        std::vector<std::uint8_t> v;
        const char* s = "the quick brown fox the quick brown fox ";
        while (v.size() < 6000)
            for (const char* p = s; *p; ++p) v.push_back((std::uint8_t)*p);
        v.resize(6000);
        fails += roundTrip("repeating text 6k", v);
    }
    {
        std::vector<std::uint8_t> v;
        std::uint32_t s = 1;
        for (int i = 0; i < 8000; ++i) { s = s * 1103515245u + 12345u; v.push_back((std::uint8_t)(s >> 16)); }
        fails += roundTrip("pseudo-random 8k", v);     // incompressible -> mild expansion
    }
    {
        // screen-like: runs + occasional change (favourable for LZ)
        std::vector<std::uint8_t> v; std::uint32_t s = 7;
        while (v.size() < 24576) {
            s = s * 1103515245u + 12345u;
            std::size_t run = 1 + ((s >> 8) % 50);
...
void usage(const char* a0)
{
    std::fprintf(stderr,
                 "Usage:\n"
                 "  %s c <in> <out>   IMPLODE-compress a memory block (file)\n"
...
int main(int argc, char** argv)
{
    try {
        ...
        if (argc == 4 && std::string(argv[1]) == "d") {
            auto in = readFile(argv[2]);
            auto out = implode::decompress(in);
            writeFile(argv[3], out);
